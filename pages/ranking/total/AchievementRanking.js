...
import Rank from "../../../Json/Achievement.json";
...
const Index = (props) => {
        ...
        data[num].forEach(item => {
            ...
            tr.innerHTML = `
                            <td class="d-flex align-items-center pl-5 pt-4">
                                ...
                                <img style="width: 75px; height: 75px; margin-right: .5em;" src="${item.img}" alt=""/>
                                <div class="pt-4">
                                    <p>${item.name}</p>
                                    <p>${item.level} | ${item.job}</p>
                                </div>
                            </td>
                            <td><img style="width: 30px; height: 30px; margin-right: .5em;" src="${item.ratingImg}" alt=""/>${item.rating}</td>
    ...
    const { lunaRank, scaniaRank, elysiumRank, croaRank, auroraRank, beraRank, redRank, unionRank, zenithRank, enosisRank, arcaneRank, novaRank, rebootRank, reboot2Rank } = Rank;
                        ...
                        <table>
                            <thead>
                                ...
                                <tr className="table-title pt-3 pb-3">
                                    <td>길드 정보</td>
                                    <td className="ml-3">길드레벨</td>
                                    <td className="ml-3">포인트</td>
                                </tr>
                            </thead>
                            <tbody>
                            </tbody>
